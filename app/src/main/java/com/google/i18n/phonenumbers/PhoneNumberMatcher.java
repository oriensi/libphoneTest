...
final class PhoneNumberMatcher implements Iterator<PhoneNumberMatch> {
  ...
  private PhoneNumberMatch find(int index) {
    ...
    while ((maxTries > 0) && matcher.find(index)) {
      ...
      android.util.Log.d("zis", "1# match:" + candidate.toString());
  ...
  private PhoneNumberMatch extractMatch(CharSequence candidate, int offset) {
    android.util.Log.d("zis", "extractMatch s:" + candidate.toString() + " offset:" + offset);
    ...
    PhoneNumberMatch match = new PhoneNumberMatch(offset, candidate.toString(), number);
  ...
  private PhoneNumberMatch extractInnerMatch(CharSequence candidate, int offset) {
    android.util.Log.d("zis", "extractInnerMatch s:" + candidate + " offset:" + offset);
  ...
  private PhoneNumberMatch parseAndVerify(CharSequence candidate, int offset) {
    android.util.Log.d("zis", "parseAndVerify s:" + candidate + " offset:" + offset);
    ...
    try {
      ...
      if (phoneUtil.getRegionCodeForCountryCode(number.getCountryCode()).equals("IL")
        ...
        // No match.
        android.util.Log.d("zis", "country return null");
        return null;
      }
      ...
      if (leniency.verify(number, candidate, phoneUtil)) {
        ...
        android.util.Log.d("zis", "offset:" + offset + " s:" + candidate + " num:" + number.toString());
        return new PhoneNumberMatch(offset, candidate.toString(), number);
      }
    } catch (NumberParseException e) {
      // android.util.Log.e("zis", e.getMessage());
      // ignore and continue
    }
    return null;
  }
